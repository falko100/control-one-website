...
imageAlt: 'De procesaanpak van ISO 9001 uitgelegd — Control One'
keywords:
  - 'ISO 9001 procesaanpak'
  - 'procesmanagement ISO 9001'
  - 'proceskaart QMS'
  - 'processen in kaart brengen ISO 9001'
draft: false
---
...
De procesaanpak houdt in dat een organisatie haar activiteiten niet als losse taken beschouwt, maar als samenhangende processen die input omzetten in output. Elk proces heeft een klant — intern of extern — die de output ontvangt en beoordeelt.

ISO 9001 verbindt de procesaanpak direct aan de Plan-Do-Check-Act-cyclus (PDCA): processen worden gepland, uitgevoerd, gemonitord en verbeterd. Dit is geen eenmalige exercitie maar een continu mechanisme.

De kern van de procesaanpak bestaat uit vier elementen:

1. **Bepalen** welke processen de organisatie nodig heeft
2. **Begrijpen** hoe processen met elkaar samenhangen
3. **Beheersen** van processen door criteria, middelen en verantwoordelijkheden vast te leggen
4. **Verbeteren** van processen op basis van prestatiemetingen

## Processen identificeren en structureren

De eerste stap is het identificeren van alle relevante processen. Veel organisaties werken met drie categorieën:

| Procestype | Omschrijving | Voorbeelden |
|---|---|---|
| Primaire processen | Leveren direct waarde voor de klant | Verkoop, productie, dienstverlening, levering |
...
- **Input** — wat is de aanleiding of het startpunt van het proces?
- **Activiteiten** — de opeenvolging van stappen
- **Output** — wat levert het proces op?
- **Klant** — wie ontvangt de output (intern of extern)?
- **Prestatie-indicatoren** — hoe meten wij of het proces goed functioneert?
- **Risico's en kansen** — relevante risico's die het proces bedreigen of kansen bieden

Een schildpaddiagram is een populaire visuele vorm voor proceskaarten: het toont de inputs, outputs, middelen, methoden, leveranciers en klanten van een proces in één overzicht.
...
Wijs voor elk proces een proceseigenaar aan — een medewerker (of functie) die verantwoordelijk is voor het beheer, de prestaties en de verbetering van het proces. Proceseigenaarschap is essentieel voor de werking van de procesaanpak in de praktijk.

## Processen monitoren en verbeteren

Een procesaanpak zonder meting is een papieren exercitie. ISO 9001 clausule 9.1 eist dat de organisatie de processen monitort, meet, analyseert en evalueert.

Stel voor elk primair proces minimaal één KPI vast. Voorbeelden:
- Doorlooptijd van offerte tot oplevering
- Percentage producten dat bij eerste controle voldoet (first pass yield)
- Klanttevredenheidsscore per projectfase
- Percentage orders geleverd op afgesproken datum

Rapporteer KPI-resultaten periodiek en gebruik ze als input voor de managementreview. Wanneer een KPI structureel buiten de target valt, is dat een signaal voor procesverbetering — vastgelegd als kans of risico en opgepakt als verbeteractie.
